...
public class AsistenteEspectaculo extends Thread {
    ...
    public void run() {
        try {
            while (!parque.parqueCerradoDefinitivamente()) {
                // Espera a que haya un espectáculo disponible
                System.out.println("[ASISTENTE_ESPECTACULO] Asistente " + id
                        + " está durmiendo, esperando próximo espectáculo...");

                // Espera hasta las próximas horas de espectáculo (9, 12, 15, 18, 21)
                boolean hayEspectaculo = parque.esperarProximoEspectaculo();
                if (!hayEspectaculo) {
                    System.out.println("[ASISTENTE_ESPECTACULO] Asistente " + id + " se retira: parque cerrado.");
                    break;
                }
                ...
                System.out.println(
                        "[ASISTENTE_ESPECTACULO] Asistente " + id + " salió del espectáculo. Se volverá a dormir.");
            }
